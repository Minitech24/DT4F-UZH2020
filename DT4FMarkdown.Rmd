---
title: "Beta and Stock Returns"
subtitle: "A Portfolio Analyis"
...
### 2. Calculation and Portoflio Building
...
We begin our analysis with the beta values of the various porftolios. The bar chart below explains us the average beta values of each portfolio. As expected portfolio 1 has got the lowest beta with a value of `r DF_mean_beta[1,2]` while portfolio 5 has the highest value of `r DF_mean_beta[1,5]`. Portfolio 3 has a beta value almost equal to 1, which should be equivalent to the beta of the market portoflio according to the CAPM.
...
##### 3.2.3 Time Series of Beta Values
...
##### 3.2.4 Summary table 
Lastly, we can inspect the mean daily returns as well as the mean standard deviation of each portfolio. The following table consolidates our findings:
...
Our findings are further consolidated in this bar chart. As we can see the average return per year for portfolio 5 is lower than for the SMI. Other than that, portfilio 4 possesses the highest yearly return followed by the other beta sorted portfolio in descending order. Moreover, even though portfolio 3 has a average beta value close to 1 it still outperforms the SMI by a significant margin. 
